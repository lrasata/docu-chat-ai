data "aws_secretsmanager_secret" "docu_chat_ai_secrets" {
  name = "${var.environment}/docu-chat-ai/secrets"
}
...
locals {
  ...
  file_upload_auth_secret = jsondecode(data.aws_secretsmanager_secret_version.docu_chat_ai_secret_values.secret_string)["API_GW_AUTH_SECRET"]
}